...
class SPCCpkDashboard(QtWidgets.QWidget):
        ...
        try:
            ...
            try:
                ...
                if os.path.exists(legacy_path):
                    ...
                    raise ImportError("無法載入 oob_module；請確保 oob_module.py 存在或可 import")
    ...
    def draw_spc_chart(self, group_name: str, chart_name: str, chart_info):
        ...
        if raw_df is None or raw_df.empty:
            ax_main.text(0.5, 0.5, "無資料", ha='center', va='center', transform=ax_main.transAxes)
            ax_box.text(0.5, 0.5, "無資料", ha='center', va='center', transform=ax_box.transAxes)
            ax_qq.text(0.5, 0.5, "無資料", ha='center', va='center', transform=ax_qq.transAxes)
            self.canvas.draw()
            return
        ...
        if use_time_axis:
            ...
            if 'point_time' in plot_df.columns and not plot_df.empty:
                ...
                if total <= 8:
                    ...
                    if tick_idx[-1] != total:
                        tick_idx.append(total)
                labels = [times[i-1].strftime('%m-%d') for i in tick_idx]
                ax.set_xticks(tick_idx)
    ...
    def _draw_box_plot(self, ax, plot_df, chart_info):
        ...
        # 創建箱型圖
        box_plot = ax.boxplot(y, patch_artist=True, notch=True, 
        ...
        if usl is not None:
            ax.axhline(y=usl, color='red', linestyle='--', linewidth=1, alpha=0.8, label='USL')
            ax.text(1.1, usl, 'USL', va='center', ha='left', fontsize=8, color='red')
        if lsl is not None:
            ax.axhline(y=lsl, color='red', linestyle='--', linewidth=1, alpha=0.8, label='LSL')
            ax.text(1.1, lsl, 'LSL', va='center', ha='left', fontsize=8, color='red')
            
        # 設定 y 軸範圍與主圖一致
        extra_vals = [v for v in [usl, lsl] if v is not None]
        if len(y) > 0:
            ymin_sel = float(np.min(y))
            ymax_sel = float(np.max(y))
        else:
